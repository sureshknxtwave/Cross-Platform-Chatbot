  ...
  return (
    <section id="courses" className="py-20 bg-cream">
      <div className="max-w-7xl mx-auto px-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2 text-sm text-gray-500">
                    <span className="flex items-center gap-1">
                      <Users className="w-3 h-3" /> {course.seats} slots
                    </span>
        ...
        {/* Extended Care Modules */}
        <div className="grid md:grid-cols-2 gap-6">
          {/* Care Programs */}
          <div className="bg-white rounded-xl p-6 border border-gray-100">
            <div className="flex items-center gap-4 mb-4">
              <img src={images.tech2} alt="Tech" className="w-16 h-16 rounded-lg object-cover" />
              <h3 className="font-heading text-xl font-semibold text-gray-800">Extended Care Programs</h3>
            </div>
            <div className="space-y-2">
              {coursesContent.mtech.map((course) => (
                <div key={course.name} className="flex justify-between text-sm py-2 border-b border-gray-50 last:border-0">
                  ...
                  <span className="text-gray-500">{course.seats} slots</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-100">
            <div className="flex items-center gap-4 mb-4">
              <img src={images.tech3} alt="Emergency" className="w-16 h-16 rounded-lg object-cover" />
              <h3 className="font-heading text-xl font-semibold text-gray-800">Emergency Module</h3>
            </div>
            <p className="text-gray-700">{coursesContent.mba.name}</p>
            <div className="flex items-center gap-4 mt-3 text-sm">
              <span className="text-gray-500">{coursesContent.mba.seats} slots</span>
              <span className="text-maroon font-medium">{coursesContent.mba.avg}</span>
            </div>
            <p className="text-gray-500 text-sm mt-2">
              Focused support for red-flag escalation and urgent care guidance
            </p>
          </div>
        </div>
      </div>
