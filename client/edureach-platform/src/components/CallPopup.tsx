...
export default function CallPopup({ open, onClose }: CallPopupProps) {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!phone || !specialty || !topic) {
      toast.error("Please fill in all fields");
      return;
    }
    setStatus("calling");
    try {
      await initiateCall({ phone, course: specialty, topic });
      setStatus("done");
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 px-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="bg-maroon rounded-t-2xl px-6 py-5">
          <h3 className="font-heading text-xl font-bold text-white">Talk to Our AI Care Assistant</h3>
          <p className="text-white/70 text-sm mt-1">Get personalized guidance on symptoms, booking and reminders</p>
        </div>
        ...
        <div className="p-6">
          {status === "form" && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Preferred Specialty *</label>
          ...
          {status === "calling" && (
            <div className="text-center py-10">
              ...
              <p className="text-gray-500 text-sm">Our AI care assistant is dialing {phone}</p>
            </div>
          )}
