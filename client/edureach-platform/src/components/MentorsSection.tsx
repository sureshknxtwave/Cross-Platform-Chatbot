...
export default function DoctorsSection({ onReachDoctors }: DoctorsSectionProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (triggered.current || !sectionRef.current || !onReachDoctors) return;
          ...
          {mentorsContent.map((mentor) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                <p className="text-xs text-gray-400">Speciality: {mentor.teaches}</p>
              </div>
            </div>
          ))}
        </div>
