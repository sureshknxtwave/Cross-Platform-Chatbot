...
export const getAppointmentInsights = async (
  _req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    const appointmentsToday = appointments.filter(
      (appt) => new Date(appt.created_at).toISOString().slice(0, 10) === todayKey,
    ).length;
  ...
  try {
    ...
    if (!patientName || !medicineName || !dosage || !frequency || !startDate || !contact) {
      res.status(400).json({
        success: false,
        message:
          "patientName, medicineName, dosage, frequency, startDate and contact are required.",
      });
      return;
    }
...
export const listMedicationReminders = async (
  _req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
